...
func main() {
	...
	logger1 := blablo.NewLogger("pref----10", string(blablo.LevelDebug))
	logger1.Info("Blablo 10 char prefix 'pref----10' example")
	logger2 := blablo.NewLogger("pref5", string(blablo.LevelDebug))
	logger2.Info("Blablo 5 char prefix 'pref5' example")
	logger3 := blablo.NewLogger("pref15---0----5", string(blablo.LevelDebug))
